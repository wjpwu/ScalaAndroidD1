...
/**
 * A class which deals with reading, parsing, and setting the zxing.camera parameters which are used to configure the zxing.camera
 * hardware.
 */
final class CameraConfigurationManager {
    ...
    /**
     * Reads, one time, values from the zxing.camera that are needed by the app.
     */
	...
	if (parameters == null) {
	    Log.w(TAG, "Device error: no zxing.camera parameters are available. Proceeding without configuration.");
	    return;
	}
	...
	camera.setParameters(parameters);
	SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(context);
	boolean currentSetting = prefs.getBoolean(Contant.KEY_FRONT_LIGHT, false);
	if (currentSetting != newSetting) {
	    SharedPreferences.Editor editor = prefs.edit();
	    editor.putBoolean(Contant.KEY_FRONT_LIGHT, newSetting);
	    editor.commit();
	}
    }
